...
namespace File_Excel_IO
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // Cài đặt: Tool -> Nuget Package manager -> manager package for solution
            // Tư duy đọc excel
            // 
            Console.OutputEncoding = System.Text.Encoding.UTF8;
            // Đường dẫn file Excel (thay đổi theo máy bạn)
            string path = @"D:\Khóa học PC Control - Vision - MVA Lab\Garbage Code\PC11\lich_bao_tri.xlsx";  // Đổi đuôi thành .xlsx
            ...
            try
            {
                ...
                using (var package = new ExcelPackage(new FileInfo(path)))
                {
                    ...
                    for (int row = 2; row <= rowCount; row++)
                    {
                        ...
                        {
                            string v = chuKy.Replace("ngày", "").Replace("day", "").Trim();
                            if (int.TryParse(v, out int soNgay))
                            {
                                canBaoTri = thoiGianQua.TotalDays >= soNgay;  // true hoặc false

                            }
                            else
                            {
                                // mười : 10
                                Console.WriteLine("Không thể lấy dữ liệu phù hợp");
                            }
                        }
                    ...
                    package.Save();   // lưu lại file excel sau khi thêm mới cột tổng tiền
                }
            }
            catch (Exception ex)
            {
